...
var sensitiveRules = []sensitivePattern{
	{
		...
		Confidence: scanner.ConfirmedConfidence, // AKIA[0-9A-Z]{16} is a precise, vendor-defined format
	...
	},
	{
		...
		Severity:   scanner.Critical,
		Confidence: scanner.ConfirmedConfidence, // PEM header is unambiguous
	...
	},
	{
		...
		Confidence: scanner.ConfirmedConfidence, // ghp_ prefix + exact length is GitHub's own format
	...
	},
	{
		CheckID:    descriptions.SensitiveDataPassword,
		Severity:   scanner.High,
		Confidence: scanner.MediumConfidence, // broad keyword heuristic — may match non-sensitive strings
	...
	},
	{
		...
		Confidence: scanner.HighConfidence, // stack trace patterns are structurally distinctive
	...
	},
	{
		...
		Severity:   scanner.Medium,
		Confidence: scanner.ConfirmedConfidence, // <title>Index of /...</title> is the exact Apache/Nginx pattern
...
func (m *SensitiveModule) Run(ctx context.Context, endpoints []crawler.Endpoint) ([]scanner.Finding, error) {
	...
	for _, ep := range endpoints {
		...
		result, err := httpclient.DefaultClient.Submit(httpclient.SubmitRequest{
			Method: "GET",
			URL:    ep.URL,
			Ctx:    ctx,
		})
...
func auditBody(targetURL, body string) []scanner.Finding {
	...
	for _, rule := range sensitiveRules {
		...
		if len(matches) > 0 {
			...
			// Sort for deterministic output: identical runs produce identical reports.
